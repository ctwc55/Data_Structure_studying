...
//기능나열
void CreateBST(int element); //이진 탐색 트리 생성 (루트 노드 생성)
void InsertNode(int element, int node); //노드 추가
...
void GetRightChild(int element, int node); //특정 노드의 오른쪽 자식노드 값 반환(출력)
void GetLeftChild(int element, int node); //특정 노드의 왼쪽 자식노드 값 반환(출력)
void CountNodeBST(int node); //이진 탐색 트리의 노드 개수의 반환(출력)
void ClearBST(); //이진 탐색 트리 초기화
